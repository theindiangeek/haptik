...
terraform state list | grep aws_eip.eip_for_bastion | while read line
do
	IP=$(terraform state show $line | grep "^public_ip[ ]*="|cut -d= -f2 | tr ' ' '\0')
...
ELB=$(terraform state show aws_elb.api | grep dns_name | cut -d= -f2| tr ' ' '\0' |sed 's/^/https:\/\//')
